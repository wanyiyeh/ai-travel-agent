...
- **身為使用者**，我想要把某一天標記為「移動日」並指定目的城市（最後一天除外），**以便**清楚呈現城市間的移動安排。
- **身為使用者**，我想要取消某天的移動日標記，**以便**修正錯誤設定。

**系統行為 / 邊界情況**
- 取消移動日標記前需二次確認。
- 設為移動日會自動清除該天的住宿安排（移動日不可有住宿）。
- 移動日在 UI 上以琥珀色標頭與跨境連接線呈現。

**主要實作位置**
- `EditableItineraryCard.tsx`：`handleStartSetTransit` / `handleConfirmSetTransit` / `handleUnsetTransit` / `saveTransitDay`
- API：`POST /api/v1/days/[dayId]/toggle-transit`
