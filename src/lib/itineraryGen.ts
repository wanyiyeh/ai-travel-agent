  ...
  const transitDayExample = isMultiCity
    ? `,
    {
      ...
      "meals": {
        ...
        "snack": { "name": "點心店名稱", "description": "簡短描述", "estimated_cost": 300 }
      }
    }`
    : "";
...
12. meals 必須填寫，早餐、午餐、晚餐、點心（snack，下午茶／甜點）各推薦一家當地實際存在的**獨立餐廳、咖啡館或甜點店**；name 必須為具體可查詢的真實店名（如「Odyssea Waterfront」、「Little Creatures Brewery」、「Hardware Société」），嚴禁使用描述性泛稱（如「當地餐廳」、「酒店餐廳」、「度假村自助早餐」、「海鮮餐廳」、「機場餐廳」、「當地咖啡館」、「當地酒吧餐廳」等）；若對偏遠地點不確定具體店名，可選擇鄰近小鎮已知的咖啡館或酒館並給出具體名稱，絕對不可以泛稱代替。description 為一句話簡介（風格、招牌菜等），estimated_cost 為每人平均消費（以 currency 為單位的整數）。
    【早餐、午晚餐、點心取材來源不同】早餐（breakfast）必須優先從下方【已驗證當地早餐地點清單】（咖啡館、麵包店、早餐專門店、早午餐餐廳等類型）選取；午餐（lunch）與晚餐（dinner）必須優先從下方【已驗證當地餐廳清單】（餐廳類）選取；點心（snack）必須優先從下方【已驗證當地點心地點清單】（咖啡館、甜點店、冰淇淋店等類型）選取；三份清單各自對應不同的 Google Place Type 或用途，不可混用——早餐、點心不可選用餐廳清單中的店家，午晚餐也不可選用早餐或點心清單中的地點；早餐清單與點心清單可能包含同一家咖啡館/麵包店，但同一家店在整份行程中仍只能被選用一次（見規則 15）。
    ...
    【點心場所限制】點心（snack）應為咖啡館、甜點店、麵包店、冰淇淋店等提供下午茶／輕食甜點的場所，不可填寫正餐型餐廳（如拉麵店、牛排館）作為點心。
    金額必須符合當地物價實際水準，各地參考範圍如下——日本（JPY）早餐約 500-1000、午餐約 1000-2000、晚餐約 2000-5000、點心約 300-600；歐元區（EUR，如法國、德國、義大利、西班牙）早餐約 8-15、午餐約 15-25、晚餐約 30-60、點心約 5-10；英國（GBP）早餐約 8-15、午餐約 15-25、晚餐約 30-60、點心約 5-10；瑞士（CHF）早餐約 15-25、午餐約 25-40、晚餐約 50-100、點心約 10-15；澳洲（AUD）早餐約 15-25、午餐約 20-35、晚餐約 40-80、點心約 10-15；紐西蘭（NZD）早餐約 15-25、午餐約 20-35、晚餐約 40-80、點心約 10-15；美國（USD）早餐約 10-20、午餐約 15-30、晚餐約 30-60、點心約 6-12；台灣（TWD）早餐約 50-120、午餐約 120-250、晚餐約 250-600、點心約 30-80；韓國（KRW）早餐約 5000-10000、午餐約 8000-15000、晚餐約 15000-40000、點心約 3000-6000；泰國（THB）早餐約 60-150、午餐約 100-250、晚餐約 200-600、點心約 40-90；新加坡（SGD）早餐約 4-10、午餐約 10-20、晚餐約 20-50、點心約 3-6；香港（HKD）早餐約 30-80、午餐約 60-130、晚餐約 150-350、點心約 20-50；越南（VND）早餐約 30000-80000、午餐約 60000-150000、晚餐約 100000-300000、點心約 20000-50000；馬來西亞（MYR）早餐約 5-15、午餐約 10-25、晚餐約 20-60、點心約 3-10；印尼（IDR）早餐約 20000-50000、午餐約 40000-100000、晚餐約 80000-250000、點心約 15000-30000；瑞典（SEK）早餐約 80-150、午餐約 150-280、晚餐約 350-800、點心約 50-100；丹麥（DKK）早餐約 60-120、午餐約 120-220、晚餐約 280-650、點心約 40-80；挪威（NOK）早餐約 90-160、午餐約 160-280、晚餐約 350-850、點心約 60-100；南非（ZAR）早餐約 80-150、午餐約 150-300、晚餐約 300-700、點心約 50-100；請根據目的地貨幣選取對應範圍，勿填寫明顯偏低的金額
...
15. 【反重複強制規則】同一行程中，同一餐廳名稱絕對不可在任何天、任何餐次重複出現，違反此規則視為生成失敗。執行方式：在腦中建立一份「已用餐廳名單」，每填入一家餐廳前先查閱名單確認該店名從未出現過，確認後才填入並立即加入名單；若想填入的店名已在名單中，必須換一家不同的餐廳。每天早、午、晚、點心四餐也必須互不重複。使用相同描述性泛稱（如「當地咖啡館」）亦視同重複，一律禁止。對於餐廳資源較少的偏遠地區（如國家公園營地、山區度假村），若同一區域連續停留多天，可使用同一露營區內不同的餐飲場所（如 Skukuza Camp 的 Cattle Baron、Berg-en-Dal 的 Mugg & Bean），或輪替附近不同小鎮的餐廳，但同一家店名仍不得重複。**移動日（isTransitDay: true）的四餐同樣適用此規則**：即使當天大部分時間在交通工具或機場，仍須給出具名真實店家（如交通樞紐內可查詢的特定餐廳、抵達城市的具名餐廳），嚴禁使用「機場餐廳」、「車站餐廳」、「高速公路休息站」等任何泛稱；若下午已抵達${isMultiCity ? returnCityName : "目的城市"}，晚餐應安排該城市的具名餐廳
