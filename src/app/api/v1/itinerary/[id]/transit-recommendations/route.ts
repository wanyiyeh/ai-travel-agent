...
export async function POST(request: Request) {
  ...
  "recommendations": [
    ...
    const systemContent = isSingleCity
      ? `你是資深的自助旅行專家。旅行者的行程以 ${originIata} 為根據地，請推薦 3 至 5 個從該城市出發值得順遊的周邊目的地，可以是城市或整個國家。${existingStops?.length ? `\n\n【重要】以下城市／機場已包含在行程中，請勿重複推薦：${existingStops.join(", ")}` : ""}
...
- topAttractions 必須是 3 個且為真實存在的知名地點`
      : `你是資深的自助旅行專家。當旅行者持有「甲地進、乙地出」的機票時，請推薦 3 至 5 個值得順路停留的目的地，可以是城市或整個國家。${existingStops?.length ? `\n\n【重要】以下城市／機場已包含在行程中，請勿重複推薦：${existingStops.join(", ")}` : ""}

回傳嚴格的 JSON 格式（不要其他文字）：
${jsonSchema}

推薦標準：
- 地理位置在起訖兩點的合理距離內即可，允許適度繞路（例如：主線旁 200-300 公里以內的熱門目的地也可推薦）
- 可以推薦單一城市（type: city），也可以推薦整個值得走訪的國家（type: country）
- 若推薦整個國家，topAttractions 填入該國 3 個最值得走訪的城市或景點
- 優先推薦有完善大眾交通連結的目的地
- 依照受歡迎程度排序（最熱門的排前面）
- topAttractions 必須是 3 個且為真實存在的知名地點`;
    ...
    const userContent = isSingleCity
      ? `行程根據地：${originIata}。\n請推薦從這個城市出發，值得順遊的周邊城市或國家（3 到 5 個）。`
      : `機票資訊：從 ${originIata} 出發，在 ${destinationIata} 結束。\n請推薦這兩個機場之間，值得順路拜訪的城市或國家（3 到 5 個）。`;
